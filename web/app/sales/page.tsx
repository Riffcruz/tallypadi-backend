...
export default function SalesPage() {
  ...
  const normalizedSubscriptionStatus = useMemo(() => {
    ...
    return (user.subscriptionStatus || '').toLowerCase() as SubscriptionStatus;
  ...
  const canAddSales = useMemo(() => {
    ...
    return false;
  }, [normalizedSubscriptionStatus, trialEndsAtMs]);

  const trialDaysLeft = useMemo(() => {
    if (normalizedSubscriptionStatus !== 'trial') return null;
  ...
  const showLockedModal = () => {
    const isTrial = normalizedSubscriptionStatus === 'trial';
  ...
  const fetchInventory = async (token: string) => {
    try {
      ...
      // Map inventory data to match frontend interface
      const raw = Array.isArray(res.data) ? res.data : (res.data?.data || []) as RawInventoryItem[];
      const mapped: InventoryItem[] = raw.map((it: RawInventoryItem) => ({
        id: String(it._id || it.id || ''),
        name: String(it.name || ''),
        stock: Number(it.stock ?? it.quantity ?? 0),
        price: Number(it.lastUnitPrice ?? it.price ?? 0),
      })).filter((x: InventoryItem) => x.id && x.name);
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        const url = data?.url || (data?.fileName ? `https://tallypadi.com/reports/${data.fileName}` : null);
        if (!url) throw new Error('No PDF url in JSON response');

        window.open(url, '_blank');
        setSuccessMsg('Opened PDF link ✅');
        return;
      }
      ...
      a.href = url;
      a.download = `Sales_Report_${dateRange.start || 'start'}_${dateRange.end || 'end'}.pdf`;
      ...
      setErrorMsg(e?.message || 'Failed to generate/download PDF.');
    } finally {
      setLoading(false);
      setTimeout(() => {
        setSuccessMsg('');
