...
export default function InventoryPage() {
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    axios.post(`${API_URL}/inventory`, newItem, { headers: { Authorization: `Bearer ${token}` }})
      .then(res => {
         return axios.get(`${API_URL}/inventory`, { headers: { Authorization: `Bearer ${token}` }});
      })
      .then(res => {
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 overflow-x-hidden min-h-screen w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className={`lg:col-span-2 bg-white p-5 rounded-2xl border shadow-sm transition-colors ${
                ...
                <form onSubmit={handleAddItem} className="flex flex-col sm:flex-row gap-3 items-end">
                    <div className="w-full sm:flex-1">
                        <label className="block text-xs font-medium text-gray-500 mb-1 ml-1">Item Name</label>
                        <input
                            type="text"
                            value={newItemName}
                    ...
                    <div className="w-full sm:w-36">
                        <label className="block text-xs font-medium text-gray-500 mb-1 ml-1">Price (₦)</label>
                        <input
                            type="number"
                            value={newItemPrice}
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
                <div>
                    <p className="text-gray-500 text-xs font-medium uppercase tracking-wider mb-1">Total Items</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <td className="px-6 py-4">
                        {editingId === item.id ? (
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {item.stock < 5 && (
                                    <span className="text-[10px] text-red-500 font-medium animate-pulse">Low Stock</span>
                                )}
                            </div>
                        )}
                    </td>
          ...
          {filteredInventory.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                  ...
                  {searchTerm && <button onClick={() => setSearchTerm('')} className="text-green-600 text-xs mt-1 hover:underline">Clear search</button>}
              </div>
          )}
        </div>
      </main>
