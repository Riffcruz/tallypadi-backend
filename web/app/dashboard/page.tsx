...
// ✅ Custom Tooltip (NO formatter prop => no TS fights)
  ...
  return (
    <div className="bg-white border border-gray-100 shadow-xl rounded-2xl px-4 py-3">
      ...
      <p className="text-lg font-black text-gray-900 tabular-nums">
        {formatCurrency(Number.isFinite(amount) ? amount : 0, currencyCode, userLocale)}
      </p>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans text-gray-900 relative overflow-x-hidden">
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 md:translate-x-0 ${
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2.5 bg-white border border-gray-200 rounded-xl md:hidden text-gray-600 shadow-sm active:scale-95 transition-transform"
            >
  ...
  <ResponsiveContainer width="100%" height="100%">
    <BarChart data={salesChart}>
      ...
      <YAxis
        ...
        tickLine={false}
        tick={{ fill: '#94a3b8', fontSize: 12, fontWeight: 600 }}
        tickFormatter={(v: number) => { // Type v explicitly
          const n = Number(v ?? 0);
          return n >= 1000 ? `${Math.round(n / 100) / 10}k` : `${n}`;
        }}
      />

      {/* ✅ No formatter prop anymore */}
      <Tooltip
          ...
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm h-[400px] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin scrollbar-thumb-gray-100">
              {inventory.slice(0, 6).map((item, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-4 rounded-2xl bg-gray-50/50 border border-gray-100 hover:bg-white hover:border-emerald-200 transition-all group"
                >
                  ...
                  <tr key={i} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-bold text-gray-900 capitalize text-sm group-hover:text-emerald-700 transition-colors truncate max-w-[250px]">
                          {t.item}
                        </span>
                      </div>
                    </td>
