...
export default function LoginScreen({ authLoading, onLogin }: LoginScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // ✅ This must match your backend login route
      // If your route is /api/login, then API_URL already includes /api, so this is `${API_URL}/login`
      ...
      // ✅ Optional: quick check that this user is admin
      // Your backend should block non-admin on /admin/* anyway, so this is just UX.
      try {
      ...
      if (status === 429) {
        Swal.fire('Too many attempts', 'Slow down and try again later.', 'error');
      } else if (status === 401) {
        Swal.fire('Invalid login', 'Wrong email/phone or password.', 'error');
      } else if (status === 403) {
        Swal.fire('Blocked', String(serverMsg), 'error');
      } else {
