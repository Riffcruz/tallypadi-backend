...
function renderReceiptPdf(doc: PdfDoc, payload: {
  ...
  items.forEach((item: any, idx: number) => {
    ...
    // Pagination check
    // Ensure thermal NEVER paginates by removing the pageH check for it
    if (!isThermal && y + rowH > pageH - 80) { // 80 buffer for bottom margin + footer
         doc.addPage();
         y = margin;
    }
  ...
  // --- TOTALS ---
  y += 16;
  if (!isThermal && y + 90 > pageH - margin) { 
      doc.addPage(); 
      y = margin; 
  }
  ...
  if (isThermal) {
      ...
      const footerY = pageH - 50; // Raised footer to prevent triggering new page
      doc.fontSize(9).fillColor(THEME_INVOICE.muted).text('Thank you for your business.', 0, footerY - 15, { align: 'center' });
