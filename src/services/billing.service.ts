...
export const initializePayment = async (user: IUser, email: string, targetPlan?: 'OGA_BOSS' | 'TYCOON') => {
    try {
        ...
        const planDetails = PLAN_CONFIG[selectedPlanType];

        if (!planDetails) {
            throw new Error(`Invalid plan type: ${selectedPlanType}`);
        }

        console.log(`💳 Initializing Paystack for ${user.phoneNumber} - Plan: ${planDetails.name} (${planDetails.amount})`);
        ...
        const response = await axios.post(
            'https://api.paystack.co/transaction/initialize',
            {
                ...
                plan: planDetails.planCode, // This ensures recurring billing if set up on Paystack
                metadata: {
                    userId: user._id.toString(),
                    phoneNumber: user.phoneNumber,
                    planType: selectedPlanType, // Store intended plan in metadata
                    custom_fields: [
                        {
                ...
                // Optional: callback_url: "https://tallypadi.com/payment/callback"
            },
            {
                headers: { 
